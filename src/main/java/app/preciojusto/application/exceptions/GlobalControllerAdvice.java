...
public class GlobalControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<?> handleUnauthorizedException(WrongCredentialsException ex) {
        ExceptionResponse response = new ExceptionResponse(HttpStatus.UNAUTHORIZED, Integer.parseInt(ex.getMessage()));
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(response);
    }

    @ExceptionHandler(ValidationException.class)
    public ResponseEntity<?> handleUnauthorizedException(ValidationException ex) {
        ExceptionResponse response = new ExceptionResponse(HttpStatus.UNAUTHORIZED, Integer.parseInt(ex.getMessage()));
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(response);
    }
